...
from imageGenerator1 import FirecrawlSearch
...
class NewsProcessor:
    ...
    def __init__(self):
        ...
        self.access_token = "EAAJP3VRQ384BPYCgZBHxjl3CGWi5R7xnNVZAbmsxsxutIcdGA1tMp2GL4Wln1AdUsjZCDqj2AOeFKvZBaHUb9mkNYZAh0j60AsiNIVVFjNFgGqBdmr0RjJFV6D6Ozw9bBZBTEdzXAJfmZBgOZBIXsp2LRcZAXzPD2v9YpsdVg9JM342RENbvs5JVWeMhSFjAchaepqeAWHOeqtFVZC73xEZCY7ZBmdDrXIw5y4uSiiDhZAqrS"
    ...
    async def process_image(self, image_urls: List[str], search_query: str) -> str:
        ...
        async with FirecrawlSearch() as searcher:
            ...
            fallback_image = image_urls[0] if image_urls else ""
        ...
        async with FirecrawlSearch() as searcher:
            ...
            for cluster_id, cluster in list(clusters.items()):
                ...
                try:
                    ...
                    query = await self.use_ai.get_query(context)
                    if query:
                        web_content_extra = await searcher.scrape_pages(query)
                        if web_content_extra:
                            context_parts += searcher.news_content
                            image_urls += searcher.image_urls
                            searcher.news_content = []
                            searcher.image_urls = []
                    ...
                    if not all([news_detail.get("category"), news_detail.get("title"), 
                        ...
                        for src in news_detail.get("citations",[]):
                            sources.append(src)
                        clusters.pop(cluster_id, None)
                        continue
                ...
                try:
                    ...
                    await self.post.Post_to_db(
                        english_cart, 
                        "http://127.0.0.1:8000/create-cart/english/",
                        "http://127.0.0.1:8000/create-detail/english/"
                    )
                    logger.info(f"English news posted for cluster {cluster_id}")
                    
                    await self.post.Post_nepali_news(
                        nepali_cart,
                        "http://127.0.0.1:8000/create-details/nepali/"
                    )
    ...
    async def retry_failed_nepali_news(self, max_retries: int = 3):
        ...
        for attempt in range(max_retries):
            ...
            for news in list(failed_nepali_news):
                try:
                    await self.post.Post_nepali_news(
                        news, "http://127.0.0.1:8000/create-details/nepali/"
                    )
